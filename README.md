# Hi there! 👋

**Quatrex / クアトレックス / クアトロ**

インフラ＆マネジメント系SE。外資IT勤務。
IT勉強会「システムエンジニア友の会」主催。
最近は **Claude Code** を活用して、個人開発を楽しんでいます。

**Certifications**: 第一種情処 / セキュアド / デスペ / CISSP / PMP

## Tech Stack

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind%20CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![Supabase](https://img.shields.io/badge/-Supabase-3FCF8E?style=flat-square&logo=supabase&logoColor=white)
![Vercel](https://img.shields.io/badge/-Vercel-000000?style=flat-square&logo=vercel&logoColor=white)

### Development Tool

![Claude Code](https://img.shields.io/badge/-Claude%20Code-D97757?style=flat-square&logo=anthropic&logoColor=white)

ほぼ全てのプロジェクトは [Claude Code](https://docs.anthropic.com/en/docs/claude-code) を使って開発しています。

## Projects

### Web Services

| Project | Description | Tech |
|---------|-------------|------|
| [ABYSS - 深海探査記録](https://into-the-abyss.vercel.app) | 深海を探査して生物を捕獲・育成する放置ゲーム | React, Supabase, Vercel |
| [ボケジャッジ](https://bokejudge.vercel.app) | AIが審査する大喜利バトル | Next.js, Supabase, Gemini |
| [相模原天気予報](https://sagamihara-weather.vercel.app) | 相模原市の天気予報サイト（和モダンデザイン） | Next.js, Supabase, 気象庁API |

### OSS / Tools

| Repository | Description |
|------------|-------------|
| [claude-code-recall](https://github.com/QuatrexEX/claude-code-recall) | Claude Code のセッション履歴を横断的に閲覧・管理できるGUIツール |
| [yt-summarizer](https://github.com/QuatrexEX/yt-summarizer) | YouTube動画の字幕を取得し、AIで要約を生成するデスクトップツール |
| [TermMatrix](https://github.com/QuatrexEX/TermMatrix) | Windows Terminalでコマンドプロンプトをグリッド表示するツール |
| [fe-exam-algorithm-study](https://github.com/QuatrexEX/fe-exam-algorithm-study) | 基本情報技術者試験 科目B アルゴリズムとプログラミング対策の学習資料 |

## Community

### システムエンジニア友の会

[![connpass](https://img.shields.io/badge/-connpass-E34E5B?style=flat-square&logo=data:image/svg+xml;base64,&logoColor=white)](https://setk.connpass.com/)

**SEやIT業界人がみんなで幸せになるための会**

2021年に設立したコミュニティです。「技術力の発揮」「やりがい」「収入」「人間関係」「時間」など、複数の観点からSEの幸せを追求しています。

- **メンバー**: 1,000名以上
- **活動**: 情報処理技術者試験対策、もくもく会、生成AIアプリ制作チャレンジなど
- **開催形式**: オンライン（Discord, X Spaces）

## Contact

[![X](https://img.shields.io/badge/-@Quatrex-000000?style=flat-square&logo=x&logoColor=white)](https://x.com/Quatrex)
